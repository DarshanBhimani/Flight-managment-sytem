...
void Insert()
{
    ...
    if(choice == 1)fp1 = fopen("data.txt","rb");
    else if(choice == 2)fp1 = fopen("data1.txt","rb");
...
void Display(int choice)
{
    ...
    printf("ID   NAME     ARRIVAL Departure CLASS  CAPACITY  STATUS  DELAY\n");
    while(fread(&fl,sizeof(flight),1,fp))
    {
        printf("%-5d%-10s",fl.flight_id,fl.Name);
        printf("%3d:%d",fl.arrive.hh,fl.arrive.mm);
        printf("%6d:%d",fl.depart.hh,fl.depart.mm);
        printf("%7s%5d%12s",fl.Class,fl.capacity,fl.status);
        printf("%5d:%d",fl.delay_time.hh,fl.delay_time.mm);
        printf("\n");
    }
    fclose(fp);
}
...
void updateFlightStatus(int data)
{
    ...
    scanf("%d",&id);
    printf("Enter the arrival time of flight in HH MM format : ");
    scanf("%d %d",&hh,&mm);
    int found = 0;
    while(fread(&fg,sizeof(flight),1,f1))
    {
...
void getLongestStay(int choice)
{
    ...
    int max = INT_MIN;
    for(int i=0;i<n1;i++){
        if(max < (abs(fl1[i].arrive.hh-fl1[i].depart.hh)+fl1[i].depart.mm-fl1[i].arrive.mm)){
            fl = fl1[i];
        }
    }
    printf("The flight with longest stay is : \n");
    printf("%-5d%-10s",fl.flight_id,fl.Name);
    printf("%3d:%d",fl.arrive.hh,fl.arrive.mm);
...
void listUnique(int choice)
{
    ...
    for(int i=0;i<k;i++){
        ...
        printf("%3d:%d",fl3[i].arrive.hh,fl3[i].arrive.mm);
        printf("%3d:%d",fl3[i].depart.hh,fl3[i].depart.mm);
        printf("%7s%5d",fl3[i].Class,fl3[i].capacity);
        printf("\n");
    }
...
void listIntersection()
{
    ...
    for(int i=0;i<n1;i++){
        ...
        if(found == 1){
            if ((fl1[i].delay_time.hh-fl2[data].delay_time.hh) >= 0 && (fl1[i].delay_time.mm-fl2[data].delay_time.mm) > 0) fl3[k++] = fl2[data];
            else fl3[k++] = fl1[i];
        }
    }
    //Sorting
